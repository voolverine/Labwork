...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		moving_object = new MovingObject(GetDC(hWnd), 300, 300);
		SetTimer(hWnd, IDC_TIMER1, 55, NULL);
		break;
	}
	case WM_TIMER: {
		switch (wParam) {
		case IDC_TIMER1:
			moving_object->Move();
			RECT erase_rect;
			erase_rect = {0, 0, 800, 600};
			InvalidateRect(hWnd, &erase_rect, true);
			UpdateWindow(hWnd);
			break;
		default:
			break;
	...
	case WM_LBUTTONUP: 
	{
		moving_object->setTarger(LOWORD(lParam), HIWORD(lParam));
		break;
	}
    case WM_PAINT:
    {
